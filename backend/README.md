# COPD Health Tracker – Backend (Lambda API)
...
1. Install dependencies and zip (from this directory):
...
2. In AWS Lambda console, open **copd-api-handler** → **Code** → **Upload from** → **.zip file**. Upload `backend/copd-api.zip`. Set **Handler** to `index.handler` (Lambda will use `index.mjs` when present). Add **Environment variables** (see above) if you use non-default table names.

3. Add the new routes in API Gateway and attach the Cognito authorizer to protected routes. See **docs/AWS_API_AND_AUTH_SETUP.md**.
